...
for distro_version in DISTROS[os_name]:
    ...
    for i in pkgs:
        ...
        if final_data.has_key(key) and str(i.source_package_version) > str(final_data[key]['version']):
            final_data[key]['version'] = str(i.source_package_version)
        elif not final_data.has_key(key):
            final_data[key] = {
    ...
    file_handler.write(json.dumps(final_data.values()))
    file_handler.close()
    ...
    if os.path.getsize('%s/%s' % (package_search.getDataFilePath(), temp_file_name)) > long(MIN_DATA_FILE_SIZE):
        try:
